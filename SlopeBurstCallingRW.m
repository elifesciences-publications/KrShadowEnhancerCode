...
elseif isfield(CompiledParticles,'cycle') %RW20200311 deal w output of newest CompiledParticles script
...
%Slope thresholds
ONThreshold=379; %Fluorescence value of 1 mRNA
OFFThreshold=-379; 

%Use thresholds to call where bursts are starting/ending
StartFramePool=[];
StartSlopePool=[];
EndSlopePool=[];
...
EndPts=min(EndPts);
EndFramePool=[1000,EndPts]; %Start with a value that would never be a frame value
...
while EndFramePool(end) ~= EndFramePool(end-1)
    ...
    if sum(SmoothParticles(n).RawSlope(EndFramePool(end):end)>=ONThreshold)==0 %If there are no more frames above ON threshold, end it
        PotentialPeaks(n).Starts=[StartFramePool];
        PotentialPeaks(n).Ends=[EndFramePool(2:end)]; %Don't include intializing value of 1000
        break
    elseif sum(SmoothParticles(n).RawSlope(StartFramePool(end):end)<=OFFThreshold)==0 %Also if no more frames below OFF threshold, end it
...
% bc don't know when that burst started
% 7/27/18
...
BurstProperties(n).Duration=BurstProperties(n).Duration(BurstProperties(n).Duration>=0);
% can add in something to find the last end burst within 50min of nc14- bc
% right now is discrepency btwn inter-burst/frequency 
...
%Deal with amplitude of first burst
...
%Deal with amplitude for rest of bursts  
    for pp=2:length(PotentialPeaks(n).Ends)
        %Subtrack the "baseline" fluo present at the start of the burst
...
BurstProperties(n).BurstAmplitude=BurstProperties(n).BurstAmplitude(BurstProperties(n).BurstAmplitude~=0); %get rid of 1st 0 in case 1st frame was an end frame bc above started at pp=2
...
%Deal with the last burst since there isn't a next ON time
...
BurstProperties(n).BurstSize(end+1)=(trapz([BurstProperties(n).ONFrames(end):BurstProperties(n).OFFFrames(end)], BurstProperties(n).SmoothTrace(BurstProperties(n).ONFrames(end):BurstProperties(n).OFFFrames(end))));
    ...
    BurstProperties(n).BurstSize=nan;
end

% Connect the burst properties with the other info about the spots in each
% nucleus
...
%Also include last time turns OFF
BurstProperties(n).LastTimeOFF = ElapsedTime(PotentialPeaks(n).Ends(end));

BurstProperties(n).NBursts=length(BurstProperties(n).BurstAmplitude);
%Frequency set btwn time of first ON and 50min into nc14 
if ((length(ElapsedTime))-nc14)<100 %If the movie does not last 50min into nc14
...
%Frequency of bursts btwn first ON and last OFF
...
%Add a fraction of "active" time that is spent actually in a burst 6/3/2019
BurstProperties(n).FractONAct=sum(BurstProperties(n).Duration)/(ElapsedTime(PotentialPeaks(n).Ends(end))-(ElapsedTime(PotentialPeaks(n).Starts(1))));
BurstProperties(n).TotalElapsedTime=ElapsedTime(end)-ElapsedTime(nc14);

%include raw fluorescence trace - added 4/17/20 RW
if exist('ShortTrace','var')
    BurstProperties(n).RawTrace=SpotDiff(n).SpotOne(nc14:end);
else
    BurstProperties(n).RawTrace=SpotDiff(n).SpotOne(nc14:(nc14+100));
end

%include nuclear fluo if tracking TF-FP
if isfield(SpotDiff,'NucFluo')
    if exist('ShortTrace','var')
        %Only keep the same frames we're keeping for spots
        BurstProperties(n).NucFluo=SpotDiff(n).NucFluo(nc14:end);
        BurstProperties(n).NucFluoBleachCorr=SpotDiff(n).NucFluoBleachCorr(nc14:end); %Add field for photobleach corrected fluo RW 20200326
        %might need to play around w smoothing parameters
        BurstProperties(n).SmoothNucFluo = smooth(SpotDiff(n).NucFluo(nc14:end),0.1,'lowess');
        BurstProperties(n).SmoothNucFluoBleachCorr = smooth(SpotDiff(n).NucFluoBleachCorr(nc14:end),0.1,'lowess');
    else
        BurstProperties(n).NucFluo=SpotDiff(n).NucFluo(nc14:(nc14+100));
        %Save photobleach corrected field too for now 3/26/2020
        BurstProperties(n).NucFluoBleachCorr = SpotDiff(n).NucFluoBleachCorr(nc14:(nc14+100));
        BurstProperties(n).SmoothNucFluo = smooth(SpotDiff(n).NucFluo(nc14:(nc14+100)),0.1,'lowess');
        %Again, saving bleach corrected field separately for now 
        BurstProperties(n).SmoothNucFluoBleachCorr = smooth(SpotDiff(n).NucFluoBleachCorr(nc14:(nc14+100)),0.1,'lowess');
    end
end

else %add to save APbin even if spot not high enough to be recorded 1/15/19
        BurstProperties(n).APBin=SpotDiff(n).APBin;
        BurstProperties(n).BurstSize=nan;
        BurstProperties(n).Frequency=nan;
end
...
%Also include last time turns OFF
BurstProperties(n+length(SpotDiff)).LastTimeOFF = ElapsedTime(PotentialPeaks2(n).Ends(end));
...
BurstProperties(n+length(SpotDiff)).FractONAct=sum(BurstProperties(n+length(SpotDiff)).Duration)/(ElapsedTime(PotentialPeaks2(n).Ends(end))-(ElapsedTime(PotentialPeaks2(n).Starts(1))));
BurstProperties(n+length(SpotDiff)).TotalElapsedTime=ElapsedTime(end)-ElapsedTime(nc14);

%include raw fluorescence trace  - added 4/17/20 RW
if exist('ShortTrace','var')
    BurstProperties(n+length(SpotDiff)).RawTrace=SpotDiff(n).SpotTwo(nc14:end);
else
    BurstProperties(n+length(SpotDiff)).RawTrace=SpotDiff(n).SpotTwo(nc14:(nc14+100));
end

%Save nuclear fluo if we're tracking that
if isfield(SpotDiff, 'NucFluo')
    if exist('ShortTrace','var')
        BurstProperties(n+length(SpotDiff)).NucFluo=SpotDiff(n).NucFluo(nc14:end);
        %Add field for bleaching corrected fluo RW 20200326
        BurstProperties(n+length(SpotDiff)).NucFluoBleachCorr = SpotDiff(n).NucFluoBleachCorr(nc14:end); 
        BurstProperties(n+length(SpotDiff)).SmoothNucFluo = smooth(SpotDiff(n).NucFluo(nc14:end),0.1,'lowess');
        BurstProperties(n+length(SpotDiff)).SmoothNucFluoBleachCorr = smooth(SpotDiff(n).NucFluoBleachCorr(nc14:end),0.1,'lowess');
    else
        BurstProperties(n+length(SpotDiff)).NucFluo=SpotDiff(n).NucFluo(nc14:(nc14+100));
        BurstProperties(n+length(SpotDiff)).SmoothNucFluo = smooth(SpotDiff(n).NucFluo(nc14:(nc14+100)),0.1,'lowess');
        %Save bleach corrected fluo info separately for now 3/26/2020
        BurstProperties(n+length(SpotDiff)).NucFluoBleachCorr = SpotDiff(n).NucFluoBleachCorr(nc14:(nc14+100));
        BurstProperties(n+length(SpotDiff)).SmoothNucFluoBleachCorr = smooth(SpotDiff(n).NucFluoBleachCorr(nc14:(nc14+100)),0.1,'lowess');
    end
end
        ...
        BurstProperties(n+length(SpotDiff)).Nucleus=SpotDiff(n).Nucleus;
        BurstProperties(n+length(SpotDiff)).Frequency=nan;
    end
